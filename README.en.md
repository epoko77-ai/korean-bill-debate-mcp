...
background workflow in `v1.1.3` is available through the MCP surface first. English users should
...
`v1.1.3` plans research against the National Assembly's official term boundaries from the 1st
...
For broad research, `v1.1.3` returns a `research_id` immediately. It first exposes candidates
